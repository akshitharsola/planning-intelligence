...
## Corpus Check
- 91 files · ~77,099 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 349 nodes · 509 edges · 58 communities detected
- Extraction: 69% EXTRACTED · 31% INFERRED · 0% AMBIGUOUS · INFERRED: 159 edges (avg confidence: 0.75)
...
4. `ApplicationCreate` - 12 edges
5. `GalwayCityScraper` - 12 edges
6. `normalize_row()` - 11 edges
7. `answer_question()` - 11 edges
8. `GalwayCountyScraper` - 10 edges
9. `search()` - 10 edges
10. `OtherRegulatoryFlags` - 9 edges
...
Cohesion: 0.09
Nodes (31): ApplicationCreate, ApplicationEventCreate, OtherRegulatoryFlags, BaseModel, _arcgis_date(), _clean_str(), _derive_status(), normalize_county_row() (+23 more)

### Community 1 - "Community 1"
Cohesion: 0.09
Nodes (25): answer_question(), _extract_filters(), _fallback_summary(), Natural-language Q&A over the applications table. Read-only: the LLM only extrac, Returns {"answer": str, "filters": dict, "applications": list, "total": int}., Returns {"answer": str, "filters": dict, "applications": list, "total": int}., Returns {"answer": str, "filters": dict, "applications": list, "total": int}., _summarize() (+17 more)
...
- **Why does `normalize_row()` connect `Community 0` to `Community 11`, `Community 4`, `Community 5`?**
  _High betweenness centrality (0.276) - this node is a cross-community bridge._
- **Why does `search()` connect `Community 9` to `Community 3`, `Community 1`, `Community 11`?**
  _High betweenness centrality (0.247) - this node is a cross-community bridge._
- **Why does `extract_location()` connect `Community 11` to `Community 0`?**
...
- **Are the 10 inferred relationships involving `ApplicationCreate` (e.g. with `Resolve stage: dedup applications by the natural key (planning_authority + appli` and `Maps a raw parser row (dict, source-specific keys) into the canonical Applicatio`) actually correct?**
  _`ApplicationCreate` has 10 INFERRED edges - model-reasoned connections that need verification._