# Graph Report - /Users/akshitharsola/Documents/AiAgentic/planning-intelligence/.claude/worktrees/city-normalizer-hardening  (2026-07-05)

## Corpus Check
- 91 files · ~79,281 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 352 nodes · 515 edges · 59 communities detected
- Extraction: 68% EXTRACTED · 32% INFERRED · 0% AMBIGUOUS · INFERRED: 163 edges (avg confidence: 0.75)
...
4. `ApplicationCreate` - 13 edges
5. `normalize_row()` - 13 edges
6. `GalwayCityScraper` - 12 edges
7. `answer_question()` - 11 edges
8. `OtherRegulatoryFlags` - 10 edges
9. `GalwayCountyScraper` - 10 edges
10. `search()` - 10 edges
...
Cohesion: 0.1
Nodes (32): ApplicationCreate, OtherRegulatoryFlags, BaseModel, _arcgis_date(), _clean_str(), _derive_status(), normalize_county_row(), County normalization reuses City's status/event mapping and date parsing (both a (+24 more)
...
Cohesion: 0.14
Nodes (19): answer_question(), _extract_filters(), _extraction_prompt(), _fallback_summary(), Natural-language Q&A over the applications table. Read-only: the LLM only extrac, Returns {"answer": str, "filters": dict, "applications": list, "total": int}., _summarize(), _today() (+11 more)

### Community 3 - "Community 3"
Cohesion: 0.11
Nodes (17): Returns {"answer": str, "filters": dict, "applications": list, "total": int}., Returns {"answer": str, "filters": dict, "applications": list, "total": int}., extract_json(), get_llm_client(), HostedApiClient, LLMClient, OllamaClient, Pluggable LLM backend for the chat feature. LLM_BACKEND env var selects the impl (+9 more)
...
Cohesion: 0.18
Nodes (15): get_distinct_authorities(), get_monthly_counts(), get_status_breakdown(), get_summary(), get_type_breakdown(), chat_ask(), dashboard(), _parse_optional_date() (+7 more)
...
## Knowledge Gaps
- **24 isolated node(s):** `A mid-batch failure (OBJECTID 3002) must not block 3003 (a later,     successful`, `Scripted client: returns queued replies in order, one per .chat() call.`, `Galway City weekly-list PDF parser — first concrete implementation of the pdf_ta`, `Open a PDF and extract all table rows as a list of dicts keyed by     column_map`, `Persisted incremental-ingestion state: County's OBJECTID watermark and City's pe` (+19 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 21`** (2 nodes): `get_database_url()`, `settings.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 22`** (2 nodes): `_reset_metrics_counters()`, `conftest.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 23`** (2 nodes): `test_application_has_natural_key_constraint()`, `test_application_natural_key.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 24`** (2 nodes): `main()`, `scaffold_tree.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 25`** (2 nodes): `renderBarChart()`, `app.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 43`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 44`** (1 nodes): `session.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 54`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 55`** (1 nodes): `Return a list of dicts describing available remote items         (e.g. PDF links`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 56`** (1 nodes): `Download the given items into self.temp_dir, return local paths.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `search()` connect `Community 10` to `Community 2`, `Community 12`, `Community 6`?**
...
- **Are the 11 inferred relationships involving `ApplicationCreate` (e.g. with `Maps a raw parser row (dict, source-specific keys) into the canonical Applicatio` and `Reject rows that don't look like real planning applications.      Guards against`) actually correct?**
  _`ApplicationCreate` has 11 INFERRED edges - model-reasoned connections that need verification._