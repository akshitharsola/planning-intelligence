# Graph Report - /Users/akshitharsola/Documents/AiAgentic/planning-intelligence  (2026-07-10)

## Corpus Check
- 97 files · ~94,594 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 403 nodes · 640 edges · 64 communities detected
- Extraction: 67% EXTRACTED · 33% INFERRED · 0% AMBIGUOUS · INFERRED: 213 edges (avg confidence: 0.74)
...
4. `normalize_row()` - 18 edges
5. `LLMClient` - 17 edges
6. `FakeLLMClient` - 15 edges
7. `run_county_ingestion()` - 14 edges
8. `ApplicationCreate` - 14 edges
9. `GalwayCityScraper` - 14 edges
10. `_cleanup()` - 13 edges
...
- `Chunked backfill runner for Galway City: processes the discovered PDF backlog in` --uses--> `GalwayCityScraper`  [INFERRED]
  /Users/akshitharsola/Documents/AiAgentic/planning-intelligence/scripts/backfill_galway_city_chunked.py → /Users/akshitharsola/Documents/AiAgentic/planning-intelligence/src/sources/galway/city/scraper.py
...
Cohesion: 0.12
Nodes (23): main(), _match_source_type(), Chunked backfill runner for Galway City: processes the discovered PDF backlog in, run_batch(), load_region_config(), Load a `config/<county>/<region>.yaml` region config file., main(), _match_source_type() (+15 more)
...
Cohesion: 0.12
Nodes (15): Returns {"answer": str, "filters": dict, "applications": list, "total": int}., Returns {"answer": str, "filters": dict, "applications": list, "total": int}., Returns {"answer": str, "filters": dict, "applications": list, "total": int}., Returns {"answer": str, "filters": dict, "applications": list, "total": int}., Returns {"answer": str, "filters": dict, "applications": list, "total": int}., Returns {"answer": str, "filters": dict, "applications": list, "total": int}., Returns {"answer": str, "filters": dict, "applications": list, "total": int}., Returns {"answer": str, "filters": dict, "applications": list, "total": int}. (+7 more)
...
## Knowledge Gaps
- **35 isolated node(s):** `A mid-batch failure (OBJECTID 3002) must not block 3003 (a later,     successful`, `Scripted client: returns queued replies in order, one per .chat() call.`, `Deletes staged Galway City PDFs from data/galway/city/raw/ once they are older t`, `Galway City weekly-list PDF parser — first concrete implementation of the pdf_ta`, `Open a PDF and extract all table rows as a list of dicts keyed by     column_map` (+30 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 23`** (2 nodes): `get_database_url()`, `settings.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 24`** (2 nodes): `_reset_metrics_counters()`, `conftest.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 25`** (2 nodes): `test_application_has_natural_key_constraint()`, `test_application_natural_key.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 26`** (2 nodes): `main()`, `scaffold_tree.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 27`** (2 nodes): `renderBarChart()`, `app.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 48`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 49`** (1 nodes): `session.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 59`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 60`** (1 nodes): `Return a list of dicts describing available remote items         (e.g. PDF links`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 61`** (1 nodes): `Download the given items into self.temp_dir, return local paths.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `normalize_row()` connect `Community 0` to `Community 3`, `Community 13`, `Community 14`?**
  _High betweenness centrality (0.315) - this node is a cross-community bridge._
- **Why does `search()` connect `Community 8` to `Community 1`, `Community 4`, `Community 14`?**
  _High betweenness centrality (0.276) - this node is a cross-community bridge._
- **Why does `extract_location()` connect `Community 14` to `Community 0`?**
...
- **Are the 14 inferred relationships involving `normalize_row()` (e.g. with `test_normalize_row_received_maps_to_application_received_event()` and `test_normalize_row_granted_sets_decision_fields()`) actually correct?**
  _`normalize_row()` has 14 INFERRED edges - model-reasoned connections that need verification._